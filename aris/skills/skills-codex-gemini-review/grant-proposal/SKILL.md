---
name: grant-proposal
description: "Draft a structured grant proposal from research ideas and literature. Supports KAKENHI (Japan), NSF (US), NSFC (China, including 面上/青年/优青/杰青/海外优青/重点), ERC (EU), DFG (Germany), SNSF (Switzerland), ARC (Australia), NWO (Netherlands), and generic formats. Use when user says \"write grant\", \"grant proposal\", \"신청서\", \"write KAKENHI\", \"연구비\", \"연구비 신청\", \"연구비 쓰기\", \"NSF proposal\", or wants to turn research ideas into a funding application."
---
...
```
/lit-survey → /lit-novelty-check → [structure design] → [draft] → /research-review → [revise] → GRANT_PROPOSAL.md
...
- No literature survey → will invoke `/lit-survey` inline in Phase 1
...
Invoke `/lit-survey` to ground the proposal in real literature, then search for competing funded projects:
...
**What this does:**
- Reuse existing surveys if `/lit-survey` was already run and notes exist
- Otherwise invoke `/lit-survey` for multi-source literature search (arXiv, Scholar, Zotero, local PDFs)
...
- Run `/lit-novelty-check` on the proposed research direction to verify the gap is real:
  ```
  /lit-novelty-check "[proposed gap statement]"
  ```
...
| `sources` | all | Literature sources | → `/lit-survey` |
| `arxiv download` | false | Download arXiv PDFs | → `/lit-survey` |
...
|-----------|:-----:|---------|
| `/lit-survey` | 1 | Literature survey (if not already done) |
| `/lit-novelty-check` | 1 | Verify the gap is real |
