...
```
/lit-survey → /idea-creator (robotics framing) → /lit-novelty-check → /research-review
...
```
/lit-survey "$ARGUMENTS — focus venues: CoRL, RSS, ICRA, IROS, RA-L, TRO, Science Robotics"
```
...
```
/lit-novelty-check "[idea description with embodiment + task family + benchmark + sensor stack + controller/policy class + sim2real angle + target venues: CoRL/RSS/ICRA/IROS/RA-L]"
```
...
**Direction**: $ARGUMENTS
**Date**: [today]
**Pipeline**: lit-survey → idea-creator (robotics framing) → lit-novelty-check → research-review
...
- [ ] Run /lit-novelty-check on the final idea wording
