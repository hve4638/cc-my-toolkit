...
- **ARXIV_DOWNLOAD = false** — When `true`, `/lit-survey` downloads the top relevant arXiv PDFs during literature survey. When `false` (default), only fetches metadata via arXiv API. Passed through to `/idea-discovery` → `/lit-survey`.
...
This internally runs: `/lit-survey` → `/idea-creator` → `/lit-novelty-check` → `/research-review`
