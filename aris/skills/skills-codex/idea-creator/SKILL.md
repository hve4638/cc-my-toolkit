...
Given a broad research direction from the user, systematically generate, validate, and rank concrete research ideas. This skill composes with `/lit-survey`, `/lit-novelty-check`, and `/research-review` to form a complete idea discovery pipeline.
...
2. **Novelty quick-check**: For each idea, do 2-3 targeted searches to see if it's already been done. Full `/lit-novelty-check` comes later for survivors.
...
1. **Novelty check**: Use the `/lit-novelty-check` workflow (multi-source search + GPT-5.4 cross-verification) for each idea
...
/lit-novelty-check "top idea"     → deep novelty verification (already done in Phase 4, but user can re-run)
