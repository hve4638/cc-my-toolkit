---
name: idea-discovery
description: "Workflow 1: Full idea discovery pipeline. Orchestrates research-lit → idea-creator → novelty-check → research-review to go from a broad research direction to validated, pilot-tested ideas. Use when user says \"아이디어 찾기 전체 프로세스\", \"idea discovery pipeline\", \"처음부터 방향 찾기\", or wants the complete idea exploration workflow."
...
- **ARXIV_DOWNLOAD = false** — When `true`, `/lit-survey` downloads the top relevant arXiv PDFs during Phase 1. When `false` (default), only fetches metadata. Passed through to `/lit-survey`.
...
1. **If arXiv URL** (e.g., `https://arxiv.org/abs/2406.04329`):
   - Invoke `/lit-arxiv "ARXIV_ID" — download` to fetch the PDF
...
Phase 1 and Phase 2 will use `REF_PAPER_SUMMARY.md` as additional context — `/lit-survey` searches for related and competing work, `/idea-creator` generates ideas that build on or improve the reference paper.

### Phase 1: Literature Survey

Invoke `/lit-survey` to map the research landscape:

```
/lit-survey "$ARGUMENTS"
```
...
- **User requests changes** (e.g., "focus more on X", "ignore Y", "too broad") → refine the search with updated queries, re-run `/lit-survey` with adjusted scope, and present again. Repeat until the user is satisfied.
...
```
/lit-novelty-check "[top idea 1 description]"
/lit-novelty-check "[top idea 2 description]"
```
...
**Direction**: $ARGUMENTS
**Date**: [today]
**Pipeline**: lit-survey → idea-creator → lit-novelty-check → research-review → research-refine-pipeline
