---
name: lit-semantic-scholar
description: Semantic Scholar API를 통해 출판된 학술지/학회 논문(IEEE, ACM, Springer 등) 검색. /lit-arxiv(프리프린트)를 보완하여 인용수, 베뉴 메타데이터, TLDR 제공. "search semantic scholar", "find IEEE papers", "find journal papers", "venue papers", "citation search" 등의 요청 시 사용.
...
This skill is the **published venue** counterpart to `/lit-arxiv`:

| Skill | Source | Best for |
|-------|--------|----------|
| `/lit-arxiv` | arXiv API | Latest preprints, cutting-edge unrefereed work |
| `/lit-semantic-scholar` | Semantic Scholar API | **Published** journal/conference papers (IEEE, ACM, Springer, etc.) with citation counts, venue info, TLDR |
...
> - `/lit-semantic-scholar "topic" - max: 20` — return up to 20 results
> - `/lit-semantic-scholar "topic" - type: journal` — only journal articles
> - `/lit-semantic-scholar "topic" - type: conference` — only conference papers
> - `/lit-semantic-scholar "topic" - min-citations: 50` — only highly-cited papers
> - `/lit-semantic-scholar "topic" - year: 2022-` — papers from 2022 onward
> - `/lit-semantic-scholar "topic" - fields: all` — remove default field-of-study filter
> - `/lit-semantic-scholar "topic" - sort: citations` — bulk search sorted by citation count
> - `/lit-semantic-scholar "DOI:10.1109/..."` — fetch a single paper by DOI
...
- If present → paper is also on arXiv. Note this in output but do NOT re-fetch via `/lit-arxiv`.
...
```text
/lit-arxiv "topic"           - search arXiv preprints (complements this search)
/lit-survey "topic"    - multi-source review: Zotero + local PDFs + arXiv + S2
/lit-novelty-check "idea"    - verify novelty against literature
```
...
- If the S2 API is unreachable, suggest using `/lit-arxiv` or `/lit-survey "topic" - sources: web` as fallback.
