---
name: lit-arxiv
description: arXiv에서 학술 논문 검색, 다운로드, 요약. "search arxiv", "download paper", "fetch arxiv", "arxiv search", "get paper pdf" 등의 요청 또는 arXiv 논문을 로컬 라이브러리에 저장하려 할 때 사용.
...
> - `/lit-arxiv "attention mechanism" - max: 20` - return up to 20 results
> - `/lit-arxiv "2301.07041" - download` - download a specific paper by ID
> - `/lit-arxiv "query" - dir: literature/` - save PDFs to a custom directory
> - `/lit-arxiv "query" - download: all` - download all result PDFs
...
```text
/lit-survey "topic"     - multi-source review: Zotero + Obsidian + local PDFs + web
/lit-novelty-check "idea"     - verify your idea is novel against these papers
```
...
- If the arXiv API is unreachable, report the error clearly and suggest using `/lit-survey` with `- sources: web` as a fallback
